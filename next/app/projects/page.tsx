...
const projects: Project[] = [
    ...
    },
    {
        ...
        description: "chromium based extension for synchronizing bookmarks by using a remote json api as the source. Example could be a gist with the following contents"
    },
]

export const metadata: Metadata = {
    title: 'Projects',
    description: 'List of the personal projects that i have developed',
}
                    ...
                    {projects.map((project, index) => {
                        return (
                            <li key={index} className={"space-x-2"}>
                                <span className={"font-bold"}>{project.name}</span><span className={"italic"}>({project.description})</span>{project.link !== undefined &&
                                <a className="link" href={project.link}>[Link]</a>}{project.github_link !== undefined &&
                                <a className="link" href={project.github_link}>[GitHub]</a>}
                            </li>
                        )
                    })}
                </ul>
                <p>I have many others on my <a href={"https://github.com/frederikhs?tab=repositories"} className="link">[GitHub page]</a></p>
            </div>
        </div>
    )
}
